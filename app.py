...
# AUTHENTICATION
# ---------------------------------------------------------
# Updated with your name and existing password hash
credentials = {
    "usernames": {
        "admin": {
            "name": "Akshit Mahajan",
...
if train_file and test_file:
    ...
    if mode == "Beginner Mode":
        ...
        if threat_percent == 0:
            ...
            st.write("⚠️ Monitor network behavior and re-check logs for specific anomalies.")
        elif threat_percent < 20:
            st.write("🚨 Investigate abnormal IPs and firewall alerts immediately.")
        else:
            st.write("🛑 Immediate security review and incident response required.")
    ...
    if mode == "Advanced Mode":
        ...
        if y_test is not None:
            ...
            # Calculate CM based on actual vs predicted
            # Note: Requires y_test to have same classes as predictions
            ...
            st.metric("Overall Model Accuracy", f"{accuracy*100:.2f}%")
        ...
        selected_attack = st.selectbox("Select Attack Type to Inspect", attack_types)
        filtered = test_data[test_data["Predicted_Class"] == selected_attack]
        st.dataframe(filtered.head(50))

        # High Risk Records
        st.subheader("Top High-Risk Records (Highest Confidence Attacks)")
        high_risk = test_data[test_data["Predicted_Class"] != "normal"].sort_values(by="Confidence", ascending=False).head(10)
    ...
    st.download_button(
        label="Download Full Security Analysis Report",
        data=csv,
    ...
    st.info("Please upload both Training and Testing datasets in the sidebar to begin analysis.")